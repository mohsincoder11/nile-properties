                    ...
                    @foreach ($clientDetails as $index => $enquiry)
                        ...
                        <div class="align-items-center" style="display: flex!important;">
                            <form id="form-{{ $enquiry->id }}"
                                action="{{ route('registrationcompleteapprove_legalclrarance_with_date') }}"
                                method="POST">
                                <div class="input-group datepicker-container">
                                    <input type="text" id="date1-{{ $enquiry->id }}"
                                        ...
                                        name="date" class="form-control datepicker" placeholder="DD/MM/YYYY" required />
                            ...
                            <button data-id="{{ $enquiry->id }}"
                                style="background-color:green; border:none; max-height:25px; margin-top:2px; margin-bottom:-5px;"
                                type="button" class="btn btn-info view-details-btn" data-placement="top" title="View">
                                <i class="fa fa-eye" style="margin-left:5px;"></i>
                            </button>
                            @if ($enquiry->is_legal_clearance == 1)
                            <button data-id="{{ $enquiry->id }}"
                                style="background-color:rgb(255, 0, 195); border:none; max-height:25px; margin-top:2px; margin-bottom:-5px;"
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.view-details-btn', function() {
                ...
                $.ajax({
                    ...
                    error: function() {
                        $("#appendbody").html(
                            '<p>An error occurred while fetching the details.</p>');
                    }
                });
            });
