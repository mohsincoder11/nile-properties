...
@section('main_container')
    ...
    <div class="page-content-wrap">
        ...
        <form action="{{ route('agent_reg_store') }}" method="post" enctype="multipart/form-data">
            @csrf
                ...
                <div class="col-md-12" style="margin-top: 2vh;">
                    <table width="100%">
                        ...
                        <tr>
                            ...
                            <td style="padding: 2px;" width="5%">
                                <input type="file" class="form-control" required name="aadhar"
                    ...
                    <tbody>
                        ...
                        @foreach ($agent as $agent)
                            <tr>
                                ...
                                <td>{{ $loop->index + 1 }}</td>
                                <td>{{ $agent->name }} / {{ $agent->email }} / {{ $agent->contact_number }} /
                                    {{ $agent->city }}
                                    / {{ $agent->address }}</td>
                                {{-- <td></td> --}}
                                <td>
                                    @php
                                        $aadharPath = asset('agent/' . $agent->aadhar);
                                        $aadharFileName = basename($agent->aadhar);
                                        $defaultImagePath = asset('images/default-image.png');
                                        $aadharExists = file_exists(public_path('agent/' . $agent->aadhar));
                                    @endphp
                                ...
                                <td>
                                    @php
                                        $panPath = asset('agent/' . $agent->pan);
                                        $panFileName = basename($agent->pan);
                                        $panExists = file_exists(public_path('agent/' . $agent->pan));
                                    @endphp
                            ...
                            <button class="btn mjks open-modal" data-toggle="modal" data-target="#popup3"
                                ...
                                class="btn btn-primary serviceareaview" id="{{ $agent->id }}"><i
                                    ...
                                    <a href="javascript:void(0)" data-bs-toggle="modal"
                                        data-bs-target="#service-area-view" class=" serviceareaview"
                                        id="{{ $agent->id }}">
                                        {{-- href="{{route('agent_reg_edit', $agent->id)}}"> --}}
                                        <button
                                            style="background-color:#3399ff; border:none; max-height:25px; margin-top:-5px; margin-bottom:-5px;"
                                            type="button" class="btn btn-info" data-toggle="tooltip"
                                            data-placement="top" title="Show"><i class="fa fa-eye"
                                                style="margin-left:5px;"></i></button><i class="fas fa-eye"></i>
                                    </a>
                                ...
                                <td>
                                    <div style="display: flex;">
                                        <div>
                                            <a onclick="openEditModal('{{ route('agent_reg_edit', $agent->id) }}')">
                                                {{-- href="{{route('agent_reg_edit', $agent->id)}}"> --}}
                                                <button
                                                    style="background-color:#3399ff; border:none; max-height:25px; margin-top:-5px; margin-bottom:-5px;"
                                                    type="button" class="btn btn-info" data-toggle="tooltip"
                                    ...
                                    <button
                                        ...
                                        <div style="margin-left: 5px;">
                                            <a onclick="openDeleteModal('{{ route('agent_destroy', $agent->id) }}')">
                                                <button
                                                    style="background-color:#ff0000; border:none; max-height:25px; margin-top:-5px; margin-bottom:-5px;"
                                                    type="button" class="btn btn-info" data-toggle="tooltip"
...
<div class="modal" id="popup3" tabindex="-1" role="dialog" aria-labelledby="smallModalHead" aria-hidden="true">
    ...
    <!-- Input field to display agent_id -->
...
@section('js')
    ...
    $(document).on('click', '.serviceareaview', function() { $("#service-area-view").modal({
            ...
            var entry_id = $(this).attr('agent_id');
