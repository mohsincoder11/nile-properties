...
<div class="page-content-wrap">
        ...
        <div class="col-md-12" style="margin-top:5px;" align="center">
            <a href="{{ route('landowner_index') }}"> <button id="on" type="button" class="btn mjks"
                    style="color:#FFFFFF; height:30px; width:auto;background-color: #ba1280; "><i
                        class="fa fa-user"></i>Land Owner</button>
            </a>
            <a href="{{ route('landowner_account') }}"> <button id="on" type="button" class="btn mjks"
                    style="color:#FFFFFF; height:30px; width:auto;background-color: #105fd5; "><i
                        class="fa fa-users"></i>Land Purchase</button>
            </a>
    ...
    <form action="{{route('land_owner_reg_store')}}" method="post" enctype="multipart/form-data">
        @csrf
            ...
            <table class="table datatable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @php
                            $aadharPath = asset('land/' . $agent->aadhar);
                            $aadharFileName = basename($agent->aadhar);
                            $defaultImagePath = asset('images/default-image.png');
                            $aadharExists = file_exists(public_path('land/' . $agent->aadhar));
                            @endphp
                        ...
                        <td>
                            @php
                            $panPath = asset('land/' . $agent->pan);
                            $panFileName = basename($agent->pan);
                            $panExists = file_exists(public_path('land/' . $agent->pan));
                            @endphp
                        ...
                        <td>
                            ...
                            <a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#service-area-view"
                                ...
                                {{-- href="{{route('land_owner_reg_edit', $agent->id)}}"> --}}
                                <button
                        ...
                        <td>
                            <div style="display: flex;">
                                <div>
                                    <a onclick="openEditModal('{{route('land_owner_reg_edit', $agent->id)}}')">
                                        {{-- href="{{route('land_owner_reg_edit', $agent->id)}}"> --}}
                                        <button
                                            style="background-color:#3399ff; border:none; max-height:25px; margin-top:-5px; margin-bottom:-5px;"
                                ...
                                {{-- <a href="{{route('land_owner_destroy', $agent->id)}}">
                                    <button
                                ...
                                <div style="margin-left: 5px;">
                                    <a onclick="openDeleteModal('{{ route('land_owner_destroy', $agent->id) }}')">
                                        <button
                                            style="background-color:#ff0000; border:none; max-height:25px; margin-top:-5px; margin-bottom:-5px;"
                                            type="button" class="btn btn-info" data-toggle="tooltip"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" id="agentIdDisplay"> --}}
                <!-- Input field to display land_owner_id -->
    ...
    <div class="modal-dialog " style="max-width:800px;">
        <div class=" modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span
                        class="sr-only">Close</span></button>
                <h4 class="modal-title" id="H4">Land Owner Details</h4>
            </div>
...
{{-- <script>
    $(document).on('click', '.serviceareaview', function() { $("#service-area-view").modal({
            ...
            var entry_id = $(this).attr('land_owner_id');
