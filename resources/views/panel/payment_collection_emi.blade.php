<!-- <div class="panel-body" style="margin-bottom:15px;">
    <table class="table datatable" id="dynamic-data-three">
        ...
        <tbody>
            ...
            @foreach ($emiPayments as $index => $emi)
            <tr>
                {{-- <td>{{ $index + 1 }}</td> --}}
                <td>{{ $emi->inst_no }}</td>
                ...
                <td>
                    ...
                    <button data-toggle="modal" data-target="#storeInstallmentModal"
                        ...
                        <i class="fa fa-rupee" style="margin-left:5px;"
                            onclick="setModalData('{{ $emi->inst_no }}', '{{ $emi->initial_enquiry_id }}')"></i> Pay
                        Installment
                    </button>
...
<div class="panel-body" style="margin-bottom:15px;">
    <table class="table datatable" id="dynamic-data-three">
        ...
        <tbody>
            @php
            $installments = $emiPayments; // Assuming $emiPayments contains all installments
            $isPreviousPaid = true; // Initialize the first installment as enabled by default
            @endphp
            ...
            <tr>
                ...
                <td>
                    ...
                    @php
                    $isPreviousPaid = false; // The current installment is pending, so set to false
                    @endphp
                    @elseif($emi->status == 'paid')
                    <button
                        style="background-color:green; border:none; max-height:25px; margin-top:-5px; margin-bottom:-5px;"
                    ...
                    @php
                    $isPreviousPaid = true; // The current installment is paid, so the next one can be enabled if
